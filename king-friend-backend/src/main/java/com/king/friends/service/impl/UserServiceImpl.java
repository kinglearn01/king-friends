...
import com.king.friends.contant.UserConstant;
import com.king.friends.exception.BusinessException;
import com.king.friends.mapper.UserMapper;
...
import com.king.friends.utils.AlgorithmUtils;
import java.util.*;
...
public class UserServiceImpl extends ServiceImpl<UserMapper, User>
    ...
    public long userRegister(UserRegisterRequest userRegisterRequest) {
        ...
        if (userRegisterRequest.getUserPassword().length() < 8 || userRegisterRequest.getUserPassword().length() < 8) {
        ...
        user.setTags(userRegisterRequest.getTags().stream().map(val->("\""+val+"\"")).toList().toString());
...
//            计算分数
        ...
        List<Pair<User, Long>> userIdList = list.stream()
                .sorted((a, b) -> (int) (a.getValue() - b.getValue()))
                .limit(num)
                .collect(Collectors.toList());
         List<Long> userVOList = userIdList.stream().map(pair->pair.getKey().getId()).collect(Collectors.toList());
        QueryWrapper<User> userQueryWrapper = new QueryWrapper<>();
        queryWrapper.in("id",userVOList);
        Map<Long,List<User>>users = this.list(userQueryWrapper).stream()
                .map(user -> getSafetyUser(user))
                .collect(Collectors.groupingBy(User::getId));
        ArrayList<User> finalUserList = new ArrayList<>();
        for (Long userId:userVOList){
            finalUserList.add(users.get(userId).get(0));
        }
     ...
     */
    @Deprecated
    public List<User> searchUserByTagSQL(List<String> tagList) {
        if(CollectionUtils.isEmpty(tagList)){
            throw new BusinessException(ErrorCode.PARAMS_ERROR);
        }
        QueryWrapper<User> queryWrapper = new QueryWrapper<>();
